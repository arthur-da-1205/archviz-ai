...
1. **Identify**: Enter an email before opening `/playground` or `/gallery`. This lightweight identity separates each user's saved images.
2. **Generate**: Type a prompt, pick a style and output size, then click "Generate Design". The backend calls Pollinations, stores the generated image on disk, and saves metadata to SQLite under that email.
...
- **Database**: SQLite (better-sqlite3) with WAL mode for concurrent access and per-email gallery ownership
